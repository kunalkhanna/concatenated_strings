    ...
    #                match = [items for items in lines if item2[len(item1)-1:len(items)+len(item1)] == items]
                    for item3 in lines:
                        ...
                        if len(item3) <= len(item2[len(item1):len(item3)+len(item1)]):
                            if item2[len(item1):len(item3)+len(item1)] == item3:  # check only for list items matching the starting letter of remaining string
...
print("Performance benchmark: binary search is ",((exec_time_non_bin-exec_time_bin)/exec_time_bin)*100, "% faster than non binary search" )
'''